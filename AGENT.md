# AGENT.md — Decision System Installer
...
You are the decision-system installer. Present the available executive agents (the 20 lines + the org-builder), ask the user which they want to install, then assemble the chosen set with the shared ontology, wire the handoff registry, and run each agent's Orientation Protocol in order.
